...
class SOTGenerator:
    ...
    def is_valid_box(self, box, img_h, img_w):
        """ 
        [质检员] 检查框是否合法。
        1. 必须完全在图像范围内 (x,y >= 0 且 x+w,y+h <= img_w,h)
        2. 尺寸必须足够大 (w,h >= 10)，避免空图崩溃
        """
        x, y, w, h = box
        
        # 1. 尺寸检查 (解决 !ssize.empty() 报错的核心)
        # SCALE=1.0 下，小于 10px 的麦穗通常没有任何纹理价值
        if w < 10 or h < 10:
            return False
            
        # 2. 边界检查 (OpenCV init 要求必须在图内)
        # 如果你的 BORDER_MARGIN 工作正常，这里通常是满足的，但也防万一
        ...
        for i, item in enumerate(pbar):
                ...
                for r, c in zip(r_ind, c_ind):
                    if iou[r, c] > MATCH_THRES:
                        ...
                        if should_reset:
                            ...
                            if not self.is_valid_box(ibox, h_small, w_small):
                                used_det.add(c)
                                continue
            ...
            for di in range(len(dets)):
                if di not in used_det:
                    ...
                    if not self.is_valid_box(init_box, h_small, w_small):
                        continue
                    t_obj = cv2.TrackerCSRT_create()
...
def worker_sot_task(args):
    vname, frame_list, direction = args
    
    # 文件名: Video_Forward.pkl
    save_path = os.path.join(Config.CACHE_DIR, f"{vname}_{direction}.pkl")
    
    # 缓存检查
    if os.path.exists(save_path):
        return f"{vname} {direction} [Skip]"
...
class DARTPipeline:
    ...
    def process(self):
        ...
        for vname, frames in video_dict.items():
            frames.sort(key=lambda x: x['frame_id'])
            tasks.append((vname, frames, "fwd"))
            tasks.append((vname, frames, "bwd"))
            
        print(f"Total Videos: {len(tasks)}")
        
        # 开启 4 个进程并行处理（根据内存调整，4K图建议4，小图建议8）
        with Pool(processes=80) as pool:
            list(tqdm(pool.imap_unordered(worker_sot_task, tasks), total=len(tasks)))
